...
typedef struct {
  ...
  void (*scallback)(uint8_t num,WEBSOCKET_TYPE_t type,char msg[][30],uint64_t len); // server callback
} ws_client_t;
...
int ws_read(ws_client_t* client,ws_header_t* header,char msg[][30]); // unmasks and returns message. populates header.
