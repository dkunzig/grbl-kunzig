...
ws_client_t ws_connect_client(struct netconn* conn,
                              ...
                              void (*scallback)(uint8_t num,WEBSOCKET_TYPE_t type,char msg[][30],uint64_t len)
                            ) {
  ws_client_t client;
  client.conn = conn;
  client.url  = url;
...
static void ws_decrypt(char msg[][30],ws_header_t header,int index) {
  if(header.param.bit.MASK) {
    for(uint64_t i=0; i<header.length; i++) {
      msg[index][i] ^= header.key.part[i%4];
    }
  }
}

static void ws_encrypt(char* msg,ws_header_t header) {
  if(header.param.bit.MASK) {
    for(uint64_t i=0; i<header.length; i++) {
...
int ws_send(ws_client_t* client,WEBSOCKET_OPCODES_t opcode,char* msg,uint64_t len,bool mask) {
  ...
  if(mask) {
    ...
    ws_encrypt(encrypt,header); // encrypt it!
...
int ws_read(ws_client_t* client,ws_header_t* header,char ret[][30]) {
  ...
  do
  {
    index += 1;
  // get the header
  header->param.pos.ZERO = buf[pos++];
  header->param.pos.ONE  = buf[pos++];

  // get the message length

  //printf("parameter bit length = %d\n",header->param.bit.LEN);
  ...
  memcpy(&ret[index],&buf[pos],header->length); // allocate the total memory
  ...
  ret[index][header->length] = '\0'; // end string
  ws_decrypt(ret,*header,index); // unencrypt, if necessary
  printf("start here ------------------- ret = %s\n",ret[index]);
 /* 
  if(header->param.bit.FIN == 0) { // if the message isn't done
    if((header->param.bit.OPCODE == WEBSOCKET_OPCODE_CONT) &&
      ...
      memcpy(client->contin,ret[index],header->length);
 ...
 */ 
    len = len - (header->length +6);
    //printf("len here is %lld\n",len - (header->length +6));
  } while (len > 0);
