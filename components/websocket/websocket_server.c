...
static void background_callback(struct netconn* conn, enum netconn_evt evt,u16_t len) {
  ...
  switch(evt) {
    case NETCONN_EVT_RCVPLUS:
      if(xQueueSendToBack(xwebsocket_queue,&conn,WEBSOCKET_SERVER_QUEUE_TIMEOUT) != pdPASS )
      {
        printf("timeout in xwebsocket_queue\n");
      }
      break;
    default:
      break;
...
static void handle_read(uint8_t num) {
  ...
  char msg[20][30];
  header.received = 0;
  ws_read(&clients[num],&header,msg);
  if(!header.received) {
    //if(msg) free(msg);
    return;
  }
  switch(clients[num].last_opcode) {
    ...
    case WEBSOCKET_OPCODE_PING:
      //ws_send(&clients[num],WEBSOCKET_OPCODE_PONG,msg1,header.length,0);
