...
#endif
			//xQueueSendToBackFromISR( debug_queue, &st,&ttt);
		} else {
...
void stepper_init()
{
	...
	#ifdef USE_RMT_STEPS
		grbl_send("[MSG:Using RMT Steps}\r\n");
		initRMT();
	#else
		grbl_send("[MSG:Using Timed Steps}\r\n");
		// make the step pins outputs
		#ifdef  X_STEP_PIN
		  	gpio_pad_select_gpio(X_STEP_PIN);
			gpio_set_direction(X_STEP_PIN, GPIO_MODE_OUTPUT);
		#endif

		#ifdef Y_STEP_PIN
		gpio_pad_select_gpio(Y_STEP_PIN);
			gpio_set_direction(Y_STEP_PIN, GPIO_MODE_OUTPUT);
		#endif
		#ifdef Y_STEP_B_PIN
		gpio_pad_select_gpio(Y_STEP_B_PIN);
			gpio_set_direction(Y_STEP_B_PIN, GPIO_MODE_OUTPUT);
		#endif

		#ifdef Z_STEP_PIN
			gpio_pad_select_gpio(Z_STEP_PIN);
			gpio_set_direction(Z_STEP_PIN, GPIO_MODE_OUTPUT);
		#endif
		
		#ifdef A_STEP_PIN
			gpio_pad_select_gpio(A_STEP_PIN);
			gpio_set_direction(A_STEP_PIN, GPIO_MODE_OUTPUT);
		#endif
	#endif

	// make the direction pins outputs
	#ifdef X_DIRECTION_PIN
		gpio_pad_select_gpio(X_DIRECTION_PIN);
		gpio_set_direction(X_DIRECTION_PIN, GPIO_MODE_OUTPUT);
		
	#endif
	#ifdef Y_DIRECTION_PIN
		gpio_pad_select_gpio(Y_DIRECTION_PIN);
		gpio_set_direction(Y_DIRECTION_PIN, GPIO_MODE_OUTPUT);
	#endif
	#ifdef Y_DIRECTION_B_PIN
		gpio_pad_select_gpio(Y_DIRECTION_B_PIN);
		gpio_set_direction(Y_DIRECTION_B_PIN, GPIO_MODE_OUTPUT);
	#endif
	#ifdef Z_DIRECTION_PIN
		gpio_pad_select_gpio(Z_DIRECTION_PIN);
		gpio_set_direction(Z_DIRECTION_PIN, GPIO_MODE_OUTPUT);
	#endif
	#ifdef A_DIRECTION_PIN
		gpio_pad_select_gpio(A_DIRECTION_PIN);
		gpio_set_direction(A_DIRECTION_PIN, GPIO_MODE_OUTPUT);
	#endif
	...
	// make the stepper disable pin an output
		...
		gpio_pad_select_gpio(STEPPERS_DISABLE_PIN);
...
#endif
#ifdef Y_DIRECTION_B_PIN
	gpio_set_level(Y_DIRECTION_B_PIN, (onMask & (1 << Y_AXIS)));
#endif
#ifdef Z_DIRECTION_PIN
	gpio_set_level(Z_DIRECTION_PIN, (onMask & (1<<Z_AXIS)));
#endif
#ifdef A_DIRECTION_PIN
	gpio_set_level(A_DIRECTION_PIN, (onMask & (1 << A_AXIS)));
#endif
}
...
#endif
#ifdef Y_STEP_B_PIN
	gpio_set_level(Y_STEP_B_PIN, (onMask & (1 << Y_AXIS)));
#endif
#ifdef Z_STEP_PIN
	gpio_set_level(Z_STEP_PIN, (onMask & (1<<Z_AXIS)));
#endif

#ifdef A1_STEP_PIN
	gpio_set_level(A1_STEP_PIN, (onMask & (1 << A_AXIS)));
}
#endif
...
void set_stepper_pins_on(uint8_t onMask)
{
	...
	if ( (ganged_mode == SQUARING_MODE_DUAL) || (ganged_mode == SQUARING_MODE_B) ) {
		gpio_set_level(X_STEP_B_PIN, (onMask & (1<<X_AXIS)));
	}
#endif
#endif
	...
	if ( (ganged_mode == SQUARING_MODE_DUAL) || (ganged_mode == SQUARING_MODE_B) ) {
		gpio_set_level(Z_STEP_B_PIN, (onMask & (1<<Z_AXIS)));
	}
#endif
#endif

#ifdef A_STEP_PIN
#ifndef A_STEP_B_PIN // if not a ganged axis
	gpio_set_level(A_STEP_PIN, (onMask & (1 << A_AXIS)));
#else // is a ganged axis
	if ((ganged_mode == SQUARING_MODE_DUAL) || (ganged_mode == SQUARING_MODE_A)) {
		gpio_set_level(A_STEP_PIN, (onMask & (1 << A_AXIS)));
	}
	...
	if ((ganged_mode == SQUARING_MODE_DUAL) || (ganged_mode == SQUARING_MODE_B)) {
		gpio_set_level(A_STEP_B_PIN, (onMask & (1 << A_AXIS)));
	}
#endif
#endif
...
#ifdef STEPPERS_DISABLE_PIN
	disabled = gpio_get_level(STEPPERS_DISABLE_PIN);
#else
