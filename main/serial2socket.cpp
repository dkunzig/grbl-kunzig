...
bool Serial_2_Socket::attachWS(void * web_socket){
	printf("serial to socket socket attached\n");
    if (web_socket) {
...
size_t Serial_2_Socket::write(const uint8_t *buffer, size_t size)
{
     //if((buffer == NULL) ||(!_web_socket)) {
            //if(buffer == NULL){
				//ets_printf("[SOCKET]No buffer");
            //}
            //if(!_web_socket){
				//ets_printf("[SOCKET]No socket");
           // }
           // return 0;
        //}
	 //Serial.print(buffer);
	 //ets_printf("serial to socket buffer = %s\n",buffer);
		...
		//ets_printf("[SOCKET]buffer size %d\n",_TXbufferSize);
        handle_flush();
#endif
    return size;
}

bool Serial_2_Socket::write_to_GRBL(uint8_t * message)
{
	
	printf("write to GRBL message = %s length = %d\n", (char*)message, strlen((char*)message));
	_RXbufferpos = 0;
	_RXbufferSize = 0;
	int i;
	int j = 0;
...
void Serial_2_Socket::handle_flush() {
    if (_TXbufferSize > 0) {
        if ((_TXbufferSize>=TXBUFFERSIZE) || ((millis()- _lastflush) > FLUSHTIMEOUT)) {
			//ets_printf("[SOCKET]need flush, buffer size %d",_TXbufferSize);
                flush();
            }
        }
}
void Serial_2_Socket::flush(void){
    if (_TXbufferSize > 0){
		...
		//ets_printf("[SOCKET]flush data, buffer size %d",_TXbufferSize);
