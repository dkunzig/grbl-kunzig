...
{	
  ...
  {
 		 ws_server_send_text_client(0,text,strlen(text));
	}
  else
  {
    printf("web socket not connected\n");
  }
}
...
// responses.
void report_status_message(uint8_t status_code,char * line)
{	
  switch(status_code) {
...
void report_feedback_message(uint8_t message_code)  // OK to send to all clients
{
    ...
    case MESSAGE_CRITICAL_EVENT:
      grbl_send("[MSG:Reset to continue]\r\n"); break;
    case MESSAGE_ALARM_LOCK:
      grbl_send("[MSG:'$H'|'$X' to unlock]\r\n"); break;
    case MESSAGE_ALARM_UNLOCK:
      grbl_send("[MSG:Caution: Unlocked]\r\n"); break;
    case MESSAGE_ENABLED:
      grbl_send("[MSG:Enabled]\r\n"); break;
    case MESSAGE_DISABLED:
      grbl_send("[MSG:Disabled]\r\n"); break;
    case MESSAGE_SAFETY_DOOR_AJAR:
      grbl_send("[MSG:Check Door]\r\n"); break;
    case MESSAGE_CHECK_LIMITS:
      grbl_send("[MSG:Check Limits]\r\n"); break;
    case MESSAGE_PROGRAM_END:
      grbl_send("[MSG:Pgm End]\r\n"); break;
    case MESSAGE_RESTORE_DEFAULTS:
      grbl_send("[MSG:Restoring defaults]\r\n"); break;
    case MESSAGE_SPINDLE_RESTORE:
      grbl_send("[MSG:Restoring spindle]\r\n"); break;
    case MESSAGE_SLEEP_MODE:
      grbl_send("[MSG:Sleeping]\r\n"); break;
#ifdef ENABLE_SD_CARD
		case MESSAGE_SD_FILE_QUIT:
			grbl_notifyf("SD print canceled", "Reset during SD file at line: %d", sd_get_current_line_number());
			grbl_sendf("[MSG:Reset during SD file at line: %d]\r\n", sd_get_current_line_number()); break;
#endif
  }  		
}
...
void report_init_message()
{
	...
	#ifdef CPU_MAP_NAME
		grbl_send("[MSG:Using cpu_map..." CPU_MAP_NAME "]\r\n");
	#endif
}
...
void report_ngc_parameters()
{
  ...
  for (coord_select = 0; coord_select <= SETTING_INDEX_NCOORD; coord_select++) {
    if (!(settings_read_coord_data(coord_select,coord_data))) {
      report_status_message(STATUS_SETTING_READ_FAIL,"setting read fail");
      return;
    }
		strcat(ngc_rpt, "[G");    
    switch (coord_select) {
...
{	
  ...
  report_status_message(status_code,"startup message"); 
}
...
void report_build_info(char *line)
{
  ...
  strcat(build_info,"]\r\n");
  grbl_send(build_info); // ok to send to all
 
}

void report_Attach_Websocket(void * _websocket)
{
	printf("web socket attached\n");
...
void report_realtime_status()
{
  ...
  float print_position[N_AXIS];
	...
	//printf("z limit pin = %d\n", gpio_set_level(Z_LIMIT_PIN));
	//printf("Y limit pin = %d\n", gpio_set_level(Y_LIMIT_PIN));
	//printf("Y limit B pin = %d\n", gpio_set_level(Y_LIMIT_B_PIN));
	//printf("X limit pin = %d\n", gpio_set_level(X_LIMIT_PIN));
