    ...
    // initial filtering by removing spaces and comments and capitalizing all letters.
      ...
      check_gcode(message);
       ...
       { 
        ...
        //printf("line sent before status return = %s\n",line);
...
void check_gcode(char * message)
{
  ...
  memset(line,0x0,80);
  for(message_counter = 0;message_counter < strlen(message);message_counter++)
  {
      //if next char in message is a return or line feed we are done
    if(message[message_counter] == '\n' || message[message_counter] == '\r')
        break;
		...
		if (line_flags) 
    {
      ...
      {
					line[char_counter++] = message[message_counter];
			}
    ...
    {    
		      if (message[message_counter] <= ' ') 
          {
					...
					else if (message[message_counter] == '(') 
          {
          ...
          else if (message[message_counter] == '[') 
          {
							...
							line[char_counter++] = message[message_counter]; // capture this character
          ...
          else if (message[message_counter] >= 'a' && message[message_counter] <= 'z') 
            { 
              // Upcase lowercase
							line[char_counter++] = message[message_counter]-'a'+'A';
						} 
          else 
            {   
							line[char_counter++] = message[message_counter];
...
void protocol_exec_rt_system()
{
  ...
  if (rt_exec) { // Enter only if any bit flag is true
	...
	case 4: grbl_send("message: EXEC_ALARM_PROBE_FAIL_INITIAL"); break;
	case 5: grbl_send("message: EXEC_ALARM_PROBE_FAIL_CONTACT"); break;
	case 6: grbl_send("message: EXEC_ALARM_HOMING_FAIL_RESET"); break;
	case 7: grbl_send("message: EXEC_ALARM_HOMING_FAIL_DOOR"); break;
	case 8: grbl_send("message: EXEC_ALARM_HOMING_FAIL_PULLOFF"); break;
	case 9: grbl_send("message: EXEC_ALARM_HOMING_FAIL_APPROACH"); break;
	}
