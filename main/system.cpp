...
void IRAM_ATTR isr_control_inputs()
{
  ...
  if (pin) {
    if (bit_istrue(pin,CONTROL_PIN_INDEX_RESET)) 
		{
			grbl_send("[MSG:Reset via control pin]\r\n"); // help debug reason for reset
      mc_reset();
    } 
          ...
          if ( line[++char_counter] == 0 ) { // Print startup lines
            for (helper_var=0; helper_var < N_STARTUP_LINE; helper_var++) {
              if (!(settings_read_startup_line(helper_var, line))) {
                report_status_message(STATUS_SETTING_READ_FAIL,"status setting read fail");
              } else {
...
uint8_t system_control_get_state()
{
  ...
  uint8_t control_state = 0;
		...
		if (gpio_get_level(CONTROL_SAFETY_DOOR_PIN)) { control_state |= CONTROL_PIN_INDEX_SAFETY_DOOR; }
	#endif
	#ifdef CONTROL_RESET_PIN
		defined_pin_mask |= CONTROL_PIN_INDEX_RESET;
		if (gpio_get_level(CONTROL_RESET_PIN)) { control_state |= CONTROL_PIN_INDEX_RESET; }
	#endif
	#ifdef CONTROL_FEED_HOLD_PIN
		defined_pin_mask |= CONTROL_PIN_INDEX_FEED_HOLD;
		if (gpio_get_level(CONTROL_FEED_HOLD_PIN)) { control_state |= CONTROL_PIN_INDEX_FEED_HOLD; }	
	#endif
	#ifdef CONTROL_CYCLE_START_PIN
		defined_pin_mask |= CONTROL_PIN_INDEX_CYCLE_START;
		if (gpio_get_level(CONTROL_CYCLE_START_PIN)) { control_state |= CONTROL_PIN_INDEX_CYCLE_START; }   
	#endif
...
uint8_t get_limit_pin_mask(uint8_t axis_idx)
{
  ...
  if (axis_idx == 3)	    { return((1 << Y_B_LIMIT_BIT)); }
